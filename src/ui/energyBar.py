...
class EnergyBar:
    ...
    def draw(self):
        ...
        # draw white interior
        self.graphik.drawRectangle(xpos + 1, ypos + 1, x - 2, height - 2, (255, 255, 255))

        # fill interior with energy
        self.graphik.drawRectangle(xpos + 1, ypos + 1, width - 2, height - 2, color)

        # draw text in center of bar
        text = str(ceil(self.player.getEnergy())) + "/" + str(self.player.getTargetEnergy())
        self.graphik.drawText(text, x/2, ypos + height/2, ceil(height) - 1, (0, 0, 0))